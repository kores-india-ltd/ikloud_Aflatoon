    ...
    public class IWL3Controller : Controller
    {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    def = new L2Helper
                    {
                        ...
                        AIStatus = ds.Tables[0].Rows[0].ItemArray[29].ToString(),
                    };
                    //def.Vftype = vftype;
                    //def.Clrtype = clrtype;
                    objectlst.Add(def);
                    ...
                    while (count > 0)
                    {
                        def = new L2Helper
                        {
                            ...
                            AIStatus = ds.Tables[0].Rows[index].ItemArray[29].ToString(),
                        };
                        //ViewBag.cnt = true;
                        //def.Vftype = vftype;
                        //def.Clrtype = clrtype;
        ...
        public ActionResult IWl3(List<string> lst, bool snd, string img, string btnClose = "default", List<Int64> idlst = null)
        {
            ...
            try
            {
                ...
                //------------------------------Calling Proc for selecting data----------------------- Session["vftype"]
