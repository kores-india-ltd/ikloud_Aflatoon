$(document).ready(function () {
    ...
    $.ajax({
        url: RootUrl + 'CreateUserNew/SelectUserRole',
        // type: 'Post',
        contentType: 'application/json; charset=utf-8',
        dataType: 'html',
        data: {},
        //async: false,
        success: function (roleLists) {
            console.log("In roleLists");
            $("#SelectRoles").empty();

            var RoleLists = JSON.parse(roleLists);
            $("#SelectRoles").append(
                $('<option></option>').val(0).html("-----Select-----"));
            $.each(RoleLists, function (i, selectRole) {
                $("#SelectRoles").append(
                    $('<option></option>').val(selectRole.ID).html(selectRole.RoleName));
            });
    ...
    $('#SelectedAccesslevel').change(function () {
        ...
        if ($("#SelectedAccesslevel").val() != null && $("#SelectedAccesslevel").val() != "") {
            ...
            if ($("#SelectedAccesslevel").val() == "ORG") {
                ...
                $.ajax({
                    url: RootUrl + 'CreateUserNew/_SelectOrgnization',
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'html',
                    //async: false,
                ...
                document.getElementById("ajaxDom").style.display = "none";
                $.ajax({
                    url: RootUrl + 'CreateUserNew/_SelectCustomer',
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'html',
                    //async: false,
                ...
                console.log('In Domain select');
                document.getElementById("ajaxorg").style.display = "none";
                document.getElementById("ajaxCust").style.display = "none";
                $.ajax({
                    url: RootUrl + 'CreateUserNew/_SelectDomains',
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'html',
                    //async: false,
    ...
    //    debugger;
    //    document.getElementById('divShowAllDetails').style.display = '';
    //});
});
$(function () {
...
function policydetails() {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: '/CreateUserNew/PolicyDetails',
            dataType: 'html',
...
function getcustdomainslist() {
    $(document).ready(function () {
        ...
        $.ajax({
            url: RootUrl + 'CreateUserNew/_getcustdomains',
            type: "POST",
            // contentType: 'application/json; charset=utf-8',
            dataType: 'html',
...
function showAllDetails() {
    ...
    var currentUserId = document.getElementById('ID').value;
    var orgNames = "";
    var custNames = "";
    var domNames = "";

    $.ajax({
        url: RootUrl + 'CreateUserNew/GetUserDetails',
        // type: 'Post',
        contentType: 'application/json; charset=utf-8',
        dataType: 'html',
        data: { accessLevel: accesslevel, currentId: currentUserId },
        async: false,
        success: function (userData) {
            debugger;

            var userDatas = JSON.parse(userData);
            orgNames = userDatas.OrganizationNames;
            custNames = userDatas.CustomerNames;
            domNames = userDatas.DomainNames;
            //$("#SelectRoles").append(
            //    $('<option></option>').val(0).html("-----Select-----"));
            //$.each(RoleLists, function (i, selectRole) {
            //    $("#SelectRoles").append(
    ...
    if (accesslevel == 'ORG') {
        document.getElementById('divOrgLevel').style.display = '';
        document.getElementById('divCustLevel').style.display = 'none';
        document.getElementById('divDomLevel').style.display = 'none';
        document.getElementById('lblOrgName').innerHTML = orgNames;
    }
    else if (accesslevel == 'CUST') {
        document.getElementById('divOrgLevel').style.display = 'none';
        document.getElementById('divCustLevel').style.display = '';
        document.getElementById('divDomLevel').style.display = 'none';
        document.getElementById('lblCustomerName').innerHTML = custNames;
    }
    else if (accesslevel == 'DOM') {
        document.getElementById('divOrgLevel').style.display = 'none';
        document.getElementById('divCustLevel').style.display = 'none';
        document.getElementById('divDomLevel').style.display = '';
        document.getElementById('lblDomainName').innerHTML = domNames;
    }

    //$(document).ready(function () {

    //    $("#dialogEditUserDetails").dialog({
    //        modal: true,
    //        draggable: true,
    //        resizable: true,
    //        //position: ['center', 'top'],
