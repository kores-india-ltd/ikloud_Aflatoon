...
$(document).ready(function () {
    ...
    if (tt.length > 0) {
        ...
        //-----------------------AI Status-------------On 15-07-2017-------------------- 
        //$("#aival").removeClass();
        //if (tt[1].AIStatus != "Not in POC") {
        //    if (tt[1].AIStatus == "Accepted") {
        //        document.getElementById("aival").classList.add("w3-highway-green");
        //    }
        //    else {
        //        document.getElementById("aival").classList.add("w3-highway-red");
        //    }
        //    document.getElementById('aival').innerHTML = tt[1].AIStatus;
        //    document.getElementById('aidisplay').style.display = "";
        //}
        //else {
        //    document.getElementById('aival').innerHTML = "";
        ...
        document.getElementById('L2rejectDecrp').innerHTML = "";
        //------------------L1 Reject---------------
        if (tt[1].L1Status == 4 || tt[1].L1Status == 5 || tt[1].L2Status == 6 || tt[1].L2Status == 7 || tt[1].L2Status == 8 || tt[1].L2Status == 9 || tt[1].L2Status == 10) {
            ...
            if (tt[1].L2Status == 6 || tt[1].L2Status == 7 || tt[1].L2Status == 8) {
    ...
    $("#btnback").click(function () {
        ...
        if (Modernizr.localstorage) {
            ...
            //---------------Call Cbs Details--------
            autogetcbsdtls(orderData.CBSClientAccountDtls, orderData.CBSJointHoldersName);
            ...
            if (tt[backcnt].L1Status == 4 || tt[backcnt].L1Status == 5 || tt[backcnt].L2Status == 6 || tt[backcnt].L2Status == 7 || tt[backcnt].L2Status == 8 || tt[backcnt].L2Status == 9 || tt[backcnt].L2Status == 10) {
                // document.getElementById('l1dec').style.display = "";

                if (tt[backcnt].L1Status == 4) {
                    $("#l1decision").removeClass();
                    document.getElementById("l1decision").innerHTML = "Y";
                    document.getElementById("l1decision").classList.add("w3-text-green");
                    document.getElementById('L1rejectDecrp').style.display = "none";
                }
                if (tt[backcnt].L2Status == 6 || tt[backcnt].L2Status == 7 || tt[backcnt].L2Status == 8) {
        ...
        if (Modernizr.localstorage) {
            ...
            if (scond == true) {
                ...
                for (var i = 0; i < cnt; i++) {
                    ...
                    //arrlist.push(orderData.AIStatus);
                }
            }
            else {
            ...
            $.ajax({
                ...
                url: RootUrl + 'TFL3Verification/TFl3',
                ...
                success: function (result) {
                    if (result == false) {
                        ...
                        window.location = RootUrl + 'TFL3Verification/Selection';
                    }
                }
    ...
    function common(val) {
        ...
        if (cnt < tt.length) {
            ...
            //-----------------------AI Status-------------On 15-07-2017--------------------
            //$("#aival").removeClass();
            //if (tt[cnt].AIStatus != "Not in POC") {
            //    if (tt[cnt].AIStatus == "Accepted") {
            //        document.getElementById("aival").classList.add("w3-highway-green");
            //    }
            //    else {
            //        document.getElementById("aival").classList.add("w3-highway-red");
            ...
            //    document.getElementById('aival').innerHTML = tt[cnt].AIStatus;
            //    document.getElementById('aidisplay').style.display = "";
            //}
            //else {
            //    document.getElementById('aival').innerHTML = "";
            ...
            //----------------------------------------------------------//
            // debugger;
            if (tt[cnt].L1Status == 4 || tt[cnt].L1Status == 5 || tt[cnt].L2Status == 6 || tt[cnt].L2Status == 7 || tt[cnt].L2Status == 8 || tt[cnt].L2Status == 9 || tt[cnt].L2Status == 10) {

                if (tt[cnt].L1Status == 4) {
                ...
                if (tt[cnt].L2Status == 6 || tt[cnt].L2Status == 7 || tt[cnt].L2Status == 8) {
                    $("#l2decision").removeClass();
                    document.getElementById("l2decision").innerHTML = "A";
                    document.getElementById("l2decision").classList.add("w3-text-green");
                }
                if (tt[cnt].L2Status == 10) {
                    $("#l2decision").removeClass();
                    document.getElementById("l2decision").innerHTML = "M";
                    document.getElementById("l2decision").classList.add("w3-text-orange");
            ...
            if (Modernizr.localstorage) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result == false) {
                            window.location = RootUrl + 'TFL3Verification/Selection?id = 1';
                        }
                        else {
                            ...
                            if (tt != null && tt != "") {
                                ...
                                document.getElementById('TransQC').innerHTML = tt[1].XMLTransCode;
                                document.getElementById('ChqDate').value = fnldate;
                                document.getElementById("iwAmt").innerHTML = addCommas(tt[1].XMLAmount); //tt[1].XMLAmount;
                                document.getElementById('accnt').value = tt[1].DbtAccountNo;
                                document.getElementById('mtrn').value = tt[1].ID;
...
function getL1Logs(iwmntr) {
    // debugger;
    $.ajax({
        url: RootUrl + 'TFL3Verification/getL1logs',
        dataType: 'html',
        data: { id: iwmntr },
        async: false,
...
function autogetcbsdtls(cbs, joint) {
    ...
    $.ajax({
        url: RootUrl + 'TFL3Verification/GetCBSDtls',
        dataType: 'html',
        type: 'POST',
        data: { ac: Acct, strcbsdls: cbs, strJoinHldrs: joint },
        success: function (data) {
            cbsbtn = true;
            signaturecall = false;
            $('#cbsdetails').html(data);
...
function getIwlogs() {
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: RootUrl + 'TFL3Verification/getIwlogs',
            dataType: 'html',
            data: { id: $("#mtrn").val() },
            success: function (data) {
                //alert(data);
