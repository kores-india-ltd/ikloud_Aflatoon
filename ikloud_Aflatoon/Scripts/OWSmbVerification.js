...
function IWLICQC() {
    ...
    //tc validation 21-12-24
    var tcflg = validYrnscodes();
    if (tcflg == false) {
        alert("Trans code is not valid !");
        document.getElementById('FinalTransCode').focus();
        document.getElementById('FinalTransCode').select();
    ...
    if (IWdecn == "A") {
        ...
        //post date /steal date added 03-12-24 start
        if (postdate <= fnldate) {
        ...
        //post date /steal date added 03-12-24 end
        ...
        if (isSortCodeValidation == "Y") {
            let resultFlg = sortCodeValidation();
            if (!resultFlg) {
                let userConfirmationSortCode = confirm("Sort Code is not valid. Do you want to proceed?");

                if (!userConfirmationSortCode) {
                    let sortQCElement = document.getElementById('FinalSortcode');
                    sortQCElement.focus();
                    sortQCElement.select();

                    // Explicitly stop the further processing
                    return false;
                }
            }
        }
        ...
        if (newAPICall == "Y" || newAPICall == "y") {
            ...
            if (isAccountStatusDormant == "Dormant") {
                ...
                //     return false;
                // }
                alert("Dormant Account not allowed..!");
                return false;
            ...
            if (isAccountStatusInactive == "Inactive") {
                ...
                {
                    var closaccflg = document.getElementById("hdvberror").value;
                    if (closaccflg == "Account is closed")
                    {
                        document.getElementById("blockkey").value = '1'
                    }
                    else
            ...
            if (isAccountFreezeStatusCode == "C") {

                alert("Credit Freeze Account Not allowed");
                return false;
            }

            if (isAccountFreezeStatusCode == "T") {

                alert("Total Freeze Account Not allowed");
                return false;
            }
        ...
        if (chkMarkP2F.checked) {
            ...
            //var p2fconfirm = confirm("P2F Instrument.Do you want to proceed?");
...
function validYrnscodes() {
    var temptrnscd = document.getElementById('FinalTransCode').value;
    var flg;
    $.ajax({
