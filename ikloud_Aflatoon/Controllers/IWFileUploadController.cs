            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    else if (file.ContentLength > 0)
                    {
                        ...
                        string[] AllowedFileExtensions = new string[] { ".dat", ".txt", ".Txt", ".TXT" };
                        ...
                        else
                        {
                            ...
                            using (var textFile = System.IO.File.OpenText(path))
                            {
                                ...
                                string line = null;
                                string sChkSrNo, sPresenBank, sChkDate, sSeqNo, sReturnReason, sCycleNo, sPresenBankRoutno, ItemSeqNo, sDraweeBank,sTransCode, sUDK, sPresentmentDate;
                                ...
                                var iwFormatName = (from f in af.ApplicationSettings
                                                    ...
                                                    select f.SettingValue).FirstOrDefault().ToString();
                                logerror(iwFormatName, iwFormatName.ToString() + " iwFormatName- >");

                                string dt = Convert.ToDateTime(Session["processdate"].ToString()).ToString("yyyy-MM-dd");
                                logerror(dt, dt.ToString() + " dt- >");
                                    ...
                                    switch (iwFormatName)
                                    {
                                        case "AxisBank":
                                            try
                                            {
                                                ...
                                                int? updateFlg = 0;
                                                iwpro.IWSearchRnt(FileID, FileSeqno, sReturnReason, sChkDate, sPresenBankRoutno, sCycleNo, ItemSeqNo, sChkSrNo, 0, ref updateFlg, ReturnReasonDescription);
                                                if (updateFlg == 1)
                                                    successrecs += 1;
                                                else
                                                {
                                        ...
                                        case "KANARADCC":
                                            try
                                            {
                                                ...
                                                int? updateFlg = 0;
                                                iwpro.IWSearchRnt(FileID, FileSeqno, sReturnReason, sChkDate, sPresenBank, sTransCode, sSeqNo, sChkSrNo, sAmount, ref updateFlg, ReturnReasonDescription);
                                                if (updateFlg == 1)
                                                    successrecs += 1;
                                                else
                                                {
                                        ...
                                        case "SIB":
                                            try
                                            {
                                                ...
                                                int? updateFlg = 0;
                                                iwpro.IWSearchRnt(FileID, FileSeqno, sReturnReason, dt, sPresenBankRoutno, sTransCode, ItemSeqNo, sChkSrNo, sAmount, ref updateFlg, ReturnReasonDescription);
                                                if (updateFlg == 1)
                                                    successrecs += 1;
                                                else
                                                {
                                        ...
                                        /////-----------------------------------
                                        case "DBS":
                                            logerror("In DBS", "In DBS" + " In DBS block- >");
                                            try
                                            {
                                                sPresenBankRoutno = line.Substring(209, 9).Trim();
                                                sChkSrNo = line.Substring(203, 6).Trim();
                                                sAmount = Convert.ToDouble(line.Substring(49, 17));
                                                sTransCode = line.Substring(218, 2).Trim();
                                                ItemSeqNo = line.Substring(181, 14).Trim();
                                                sPresentmentDate = line.Substring(39, 10).Trim();
                                                sChkDate = line.Substring(195, 8).Trim();
                                                sReturnReason = line.Substring(89, 2);
                                                string newDate = line.Substring(199, 4).Trim() + "-" + line.Substring(197, 2).Trim() + "-" + line.Substring(195, 2).Trim();
                                                string dt1 = Convert.ToDateTime(newDate.ToString()).ToString("yyyy-MM-dd");
                                                ReturnReasonDescription = line.Substring(220, 50).Trim();

                                                logerror(sPresenBankRoutno, sPresenBankRoutno.ToString() + " sPresenBankRoutno- >");
                                                logerror(sChkSrNo, sChkSrNo.ToString() + " sChkSrNo- >");
                                                logerror(sAmount.ToString(), sAmount.ToString() + " sAmount- >");
                                                logerror(sTransCode, sTransCode.ToString() + " sTransCode- >");
                                                logerror(ItemSeqNo, ItemSeqNo.ToString() + " ItemSeqNo- >");
                                                logerror(sPresentmentDate, sPresentmentDate.ToString() + " sPresentmentDate- >");
                                                logerror(sChkDate, sChkDate.ToString() + " sChkDate- >");
                                                //logerror(ReturnReasonDescription, ReturnReasonDescription.ToString() + " ReturnReasonDescription- >");
                                                ...
                                                if (af.ItemReturnReasons.Where(r => r.CBS_REASON_CODE == line.Substring(89, 2)).FirstOrDefault() != null)
                                                    sReturnReason = af.ItemReturnReasons.Where(r => r.CBS_REASON_CODE == line.Substring(89, 2)).FirstOrDefault().RETURN_REASON_CODE;
                                                else
                                                {
                                                    StreamWriter str = new StreamWriter(path.Replace(fileName.Substring(fileName.LastIndexOf('.')), ".err"), true, System.Text.Encoding.Default);
                                                    str.WriteLine("Return Reason Code " + line.Substring(89, 2) + " Not Available in Mapper At Line - " + line);
                                                    str.Close();
                                                    errFlg = true;
                                                }
                                                logerror(sReturnReason, sReturnReason.ToString() + " sReturnReason- >");
                                                logerror(dt1, dt1.ToString() + " dt1- >");

                                                int? updateFlg = 0;
                                                iwpro.IWSearchRnt(FileID, FileSeqno, sReturnReason, dt1, sPresenBankRoutno, sTransCode, ItemSeqNo, sChkSrNo, sAmount, ref updateFlg, ReturnReasonDescription);
                                                logerror(updateFlg.ToString(), updateFlg.ToString() + " updateFlg- >");
                                                if (updateFlg == 1)
                                                    successrecs += 1;
                                                else
                                                {
                                        ...
                                        //=========================================
                                        case "TNSA":
                                            logerror("In TNSA", "In TNSA" + " In TNSA block- >");
                                            try
                                            {
                                                sPresenBankRoutno = line.Substring(0, 9).Trim();
                                                sTransCode = line.Substring(9, 2).Trim();
                                                ItemSeqNo = line.Substring(11, 10).Trim();
                                                sChkSrNo = line.Substring(21, 6).Trim();
                                                
                                                //sTransCode = line.Substring(65, 2).Trim();
                                                sAmount = Convert.ToDouble(line.Substring(67, 13)) / 100;
                                                string newDate = line.Substring(84, 4).Trim() + "-" + line.Substring(82, 2).Trim() + "-" + line.Substring(80, 2).Trim();
                                                string dt1 = Convert.ToDateTime(newDate.ToString()).ToString("yyyy-MM-dd");
                                                sReturnReason = line.Substring(90, 2);
                                                ReturnReasonDescription = line.Substring(92).Trim();
                                                ...
                                                if (af.ItemReturnReasons.Where(r => r.CBS_REASON_CODE == line.Substring(90, 2)).FirstOrDefault() != null)
                                                    sReturnReason = af.ItemReturnReasons.Where(r => r.CBS_REASON_CODE == line.Substring(90, 2)).FirstOrDefault().RETURN_REASON_CODE;
                                                else
                                                {
                                                    StreamWriter str = new StreamWriter(path.Replace(fileName.Substring(fileName.LastIndexOf('.')), ".err"), true, System.Text.Encoding.Default);
                                                    str.WriteLine("Return Reason Code " + line.Substring(90, 2) + " Not Available in Mapper At Line - " + line);
                                                    str.Close();
                                                    errFlg = true;
                                                }
                                                logerror(sReturnReason, sReturnReason.ToString() + " sReturnReason- >");
                            ...
                            logerror(errFlg.ToString(), errFlg.ToString() + " errFlg- >");
                            ModelState.Clear();
                            if (errFlg)
                            {
