...
namespace ikloud_Aflatoon.Controllers
{
    public class OWL2Controller : Controller
    {
        ...
        public ActionResult OWL2Chq(int id = 0, int DomainId = 0, string branchCode = null)
        {
            ...
            //clearing type
            //  ViewBag.ctsnocts = new SelectList(af.ClearingType, "Code", "Name").ToList(); //added 02-12-24
            ViewBag.L2L3Header = "";
            if (id == 99) { ViewBag.L2L3Header = "L2 VF WithOut Slip"; }else if (id == 98) { ViewBag.L2L3Header = "L3 VF WithOut Slip"; }
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    def = new L2verificationModel
                    {
                       ...
                       // SrcFndsDescription= ds.Tables[0].Rows[0]["SrcFndsDescription"].ToString(),
                       // NROSrcFndsDescription= ds.Tables[0].Rows[0]["NROSrcFndsDescription"].ToString(),
                        callby = "Cheq",
                    };
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if (jObject["accountClosedFlag"].ToString().Trim().ToUpper() != "Y")
                                    {
                                        ...
                                        {
                                            Session["modeOfOperation"] = jObject["modeOfOperation"].ToString().Trim();
                                        }
                                        else
                                        {
                                            Session["modeOfOperation"] = "";
                                        }
                                        if (jObject["staffIndicator"] != null)
                                        {
                                            Session["staffIndicator"] = (bool)jObject["staffIndicator"] ? "Y" : "N";
                                        }
                                        else
                                        {
                                            Session["staffIndicator"] = "";
                                        }

                                        if (jObject["branchCode"] != null)
                                        {
                                            Session["branchCode"] = jObject["branchCode"].ToString().Trim();
                                        }
                                        else
                                        {
                                            Session["branchCode"] = "";
                                        ...
                                        if (jObject["productType"] != null)
                                        {
                                            Session["OfficeAcc"] = jObject["productType"].ToString() == "OAB" ? "Y" : "N";
                                        }
                                        else
                                        {
                                            Session["OfficeAcc"] = "";
                                        ...
                                        Session["AllApiData"] = Session["SourceCustomerId"].ToString() + "|" + Session["AccountCurrency"].ToString() + "|" + Session["IsOpenedDateOld"].ToString() + "|" + Session["productCode"].ToString() + "|" + Session["productType"].ToString() + "|" + Session["accountBalances"].ToString() + "|" + Session["modeOfOperation"].ToString() + "|" + Session["AccountStatus"].ToString() + "|" + Session["staffIndicator"].ToString() + "|" + Session["FreezeStatusCode"].ToString() + "|" + openDate + "|" + Session["branchCode"].ToString() + "|" + Session["OfficeAcc"].ToString();
                                        logerrorInCatch("in OWL2 GETCBSDetailsWITHAPI==LN2778==>AllApiData===>", Session["AllApiData"].ToString());
        ...
        public JsonResult GetSourceofFundsFromNRE_NRO(Int64 ID = 0, Int64 RecordID = 0,Int64 RawDataId=0, string schemeType = null)
        {
            SqlDataAdapter SQLDA = new SqlDataAdapter("GetSourceofFundsFromNRE_NRO", con);
            SQLDA.SelectCommand.CommandType = CommandType.StoredProcedure;
            SQLDA.SelectCommand.Parameters.AddWithValue("@ID", ID);
            SQLDA.SelectCommand.Parameters.AddWithValue("@RecordID", RecordID);
            ...
            SQLDA.SelectCommand.Parameters.AddWithValue("@VFType", Session["VFType"].ToString()); 
