...
{
    public class TFL1verificationController : Controller
    {
        //
        // GET: /TFL1verification/
        ...
        public ActionResult Index(string clrtype = null)
        {
            ...
            try
            {
                ...
                SqlDataAdapter adp = new SqlDataAdapter("TFL1Verification", con);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    def = new IWTempL1VerificationModel
                    {
                        ...
                        FrontGreyImagePath = ds.Tables[0].Rows[0].ItemArray[8].ToString().Replace((string)Session["SrcWebIP"], (string)Session["DestWepIP"]).Replace((string)Session["SrcWebName"], (string)Session["DestWebName"]),
                        FrontTiffImagePath = ds.Tables[0].Rows[0].ItemArray[9].ToString().Replace("tif", "jpg").Replace((string)Session["SrcWebIP"], (string)Session["DestWepIP"]).Replace((string)Session["SrcWebName"], (string)Session["DestWebName"]),
                        CBSClientAccountDtls = ds.Tables[0].Rows[0].ItemArray[10].ToString(),
                        CBSJointHoldersName = ds.Tables[0].Rows[0].ItemArray[11].ToString(),
                        BackTiffImagePath = ds.Tables[0].Rows[0].ItemArray[12].ToString().Replace("tif", "jpg").Replace((string)Session["SrcWebIP"], (string)Session["DestWepIP"]).Replace((string)Session["SrcWebName"], (string)Session["DestWebName"]),
                    };
                    objectlst.Add(def);
                    //------------------------END------------------------//
                    int index = 0;
                    ...
                    while (count > 0)
                    {
                        def = new IWTempL1VerificationModel
                        {
                            ...
                            FrontGreyImagePath = ds.Tables[0].Rows[index].ItemArray[8].ToString().Replace((string)Session["SrcWebIP"], (string)Session["DestWepIP"]).Replace((string)Session["SrcWebName"], (string)Session["DestWebName"]),
                            FrontTiffImagePath = ds.Tables[0].Rows[index].ItemArray[9].ToString().Replace("tif", "jpg").Replace((string)Session["SrcWebIP"], (string)Session["DestWepIP"]).Replace((string)Session["SrcWebName"], (string)Session["DestWebName"]),
                            CBSClientAccountDtls = ds.Tables[0].Rows[index].ItemArray[10].ToString(),
                            CBSJointHoldersName = ds.Tables[0].Rows[index].ItemArray[11].ToString(),
                            BackTiffImagePath = ds.Tables[0].Rows[index].ItemArray[12].ToString().Replace("tif", "jpg").Replace((string)Session["SrcWebIP"], (string)Session["DestWepIP"]).Replace((string)Session["SrcWebName"], (string)Session["DestWebName"]),
                        };
                        //ViewBag.cnt = true;
                        objectlst.Add(def);
                        count = count - 1;
        ...
        [HttpPost]
        public ActionResult TFl1(List<string> lst, bool snd, string img = null, string btnClose = "default", List<Int64> idlst = null)
        {
            if (Session["uid"] == null)
            {
                return RedirectToAction("Error", "Error", new { msg = "Session Expired" });
            ...
            try
            {
                ...
                if (ttcnt == 1)
                {
                    for (int i = 0; i < ttcnt; i++)
                    {
                        ...
                        iwpro.UpdateTFL1Verification(id, uid, lst[4].ToString(), lst[5].ToString(), lst[6].ToString(), lst[7].ToString(), lst[1].ToString(), Convert.ToDouble(lst[2]), lst[3].ToString(), stu, resncode, rejctdecrptn, cbdclnts, cbdJointdtls, @Session["LoginID"].ToString());
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    def = new IWTempL1VerificationModel
                    {
                        ...
                        Date = lst[3].ToString(),
                        sttsdtqc = lst[8],// Convert.ToBoolean(lst[8]),
